...
def perform_list_backups(args: argparse.Namespace):
    if hasattr(args, 'cloud') and args.cloud:
        ...
        if not os.path.isdir(archive_dir):
            ...
            try:
                archives = [f for f in os.listdir(archive_dir) if os.path.isfile(os.path.join(archive_dir, f)) and f.endswith('.gemini-manager.tar.gz')]
                if not archives:
        ...
        if not os.path.isdir(dir_backup_path):
            ...
            try:
                dir_backups = [d for d in os.listdir(dir_backup_path) if os.path.isdir(os.path.join(dir_backup_path, d)) and '.gm' in d]
                if not dir_backups:
