...
def do_update():
    ...
    run_cmd_safe("rm -f /usr/bin/gm", timeout=5, detect_reset_time=False)

    # Query npm global root
    rc, npm_root, npm_err = run_cmd_safe(
        "npm root -g", timeout=8, capture=True, detect_reset_time=False
    )
    npm_root = (npm_root or "").strip()
    if rc != 0 or not npm_root:
        cprint(
            NEON_YELLOW,
            f"[WARN] Could not determine npm global root (rc={rc}). Falling back to /usr/lib/node_modules",
        )
        npm_root = "/usr/lib/node_modules"

    #     google_pkg_dir = os.path.join(npm_root, "@google") # umcomment if current version is not working
    google_pkg_dir = os.path.join(npm_root, "@google", "gm-cli")
    ...
    if os.path.exists(npm_root):
        rc_ls, ls_out, ls_err = run_cmd_safe(
            f"ls -la {npm_root}", timeout=6, capture=True, detect_reset_time=False
        )
        if rc_ls == 0:
    ...
    # Install / update gm CLI
    cprint(NEON_YELLOW, "[INFO] Running npm install -g @google/gm-cli ...")
    rc_install, out_install, err_install = run_cmd_safe(
        "npm install -g @google/gm-cli",
        timeout=300,
        capture=True,
    ...
    if rc_install == 0:
        ...
        rc_force, out_force, err_force = run_cmd_safe(
            "npm install -g @google/gm-cli --force",
            timeout=300,
            capture=True,
            detect_reset_time=False,
        )
        ...
        if rc_force == 0:
            ...
            rc2, out2, err2 = run_cmd_safe(
                "npm install -g --unsafe-perm @google/gm-cli",
                timeout=300,
                capture=True,
                detect_reset_time=False,
            )
            if rc2 == 0:
                ...
                rc_bin, npm_bin, _ = run_cmd_safe(
                    "npm bin -g", timeout=6, capture=True, detect_reset_time=False
                )
                ...
                if npm_bin:
                    ...
                    cprint(
                        NEON_YELLOW,
                        f"[TIP] If gm is installed here, you can symlink: ln -sf {npm_bin}/gm /usr/bin/gm",
                    )
    # done
...
def do_check_update():
    ...
    # Quick check: is gm on PATH?
    rc_path, gem_path, _ = run_cmd_safe(
        "command -v gm", timeout=3, capture=True, detect_reset_time=False
    )
    gem_path = (gem_path or "").strip()
    if rc_path != 0 or not gem_path:
        cprint(NEON_RED, "[ERROR] 'gm' not found on PATH. Is it installed?")
        cprint(NEON_YELLOW, "Try: npm install -g @google/gm-cli  or check your PATH.")
        return

    cprint(NEON_CYAN, f"[INFO] Found gm at: {NEON_GREEN}{gem_path}")
    ...
    rc_inst, installed, err_inst = run_cmd_safe(
        "gm --version < /dev/null", timeout=30, capture=True, detect_reset_time=False
    )
    installed = (installed or "").strip()
    if rc_inst != 0 or not installed:
        cprint(
            NEON_RED,
            "[ERROR] Gemini is installed but `gm --version` failed or timed out.",
        )
        cprint(NEON_YELLOW, f"[DEBUG] gm --version rc={rc_inst} stderr: {err_inst}")
        cprint(
            NEON_YELLOW,
            "You can try running `gm --version` manually to see interactive prompts.",
        )
        return

    # Get latest package version from npm
    rc_latest, latest, err_latest = run_cmd_safe(
        "npm view @google/gm-cli version",
        timeout=10,
        capture=True,
        detect_reset_time=False,
    )
    ...
    if rc_latest != 0 or not latest:
        ...
        cprint(NEON_CYAN, "Try: npm view @google/gm-cli version --json")
        latest = "(unknown)"
    ...
    cprint(NEON_MAGENTA, "\n⚡ Update available!")
    choice = (
        input(NEON_YELLOW + "Do you want to update? (y/n): " + RESET).strip().lower()
    )
    if choice == "y":
        do_update()
    else:
