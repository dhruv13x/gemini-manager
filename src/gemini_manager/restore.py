...
def is_backup_archive(filename: str) -> bool:
    return filename.endswith(".gemini-manager.tar.gz") or filename.endswith(
        ".gemini-manager.tar.gz.gpg"
    )
...
def perform_restore(args: argparse.Namespace):
    ...
    if hasattr(args, "cloud") and args.cloud:
        ...
        if hasattr(args, "auto") and args.auto:
            ...
            if not rec:
                cprint(
                    NEON_RED, "No 'Green' (Ready) accounts available for auto-switch."
                )
                sys.exit(1)
            ...
            if not candidates:
                cprint(
                    NEON_RED,
                    f"No backups found in cloud for recommended account: {target_email}",
                )
                sys.exit(1)
            ...
            cprint(
                NEON_GREEN,
                f"Selected latest cloud backup for {target_email}: {target_file_name}",
            )
            ...
            if target_file_name:
                ...
                print(
                    f"Error: Specified archive '{wanted_name}' not found in Cloud bucket."
                )
                sys.exit(1)
        else:
    ...
    # ---------------------------------------
        ...
        if not latest_archive:
            cprint(
                NEON_RED,
                f"No backups found locally for recommended account: {target_email}",
            )
            sys.exit(1)
        ...
        cprint(
            NEON_GREEN,
            f"Selected latest local backup for {target_email}: {from_archive}",
        )
        ...
        if not os.path.exists(user_path):
            ...
            if os.path.exists(path_in_search_dir):
                ...
                print(
                    f"Specified --from-archive not found at '{user_path}' or in search directory '{search_dir}'"
                )
                sys.exit(1)
        else:
        ...
        if not oldest_archive:
            print(
                f"No timestamped backup archives (*.gemini-manager.tar.gz) found in {sd}"
            )
            sys.exit(1)
    ...
    try:
        ...
        try:
            ...
            if not getattr(args, "dry_run", False):
                diff_proc = run(
                    f"diff -r {shlex_quote(tmp_dest)} {shlex_quote(src_for_copy)}",
                    capture=True,
                    check=False,
                )
            ...
            if os.path.exists(dest) and not args.force:
                ...
                if args.dry_run:
                    cprint(
                        NEON_YELLOW,
                        f"[DRY-RUN] Would move existing {dest} to {bakname}",
                    )
                else:
                    cprint(
                        NEON_YELLOW,
                        f"Backing up existing configuration to {bakname}...",
                    )
            ...
            if not getattr(args, "dry_run", False):
                print(
                    "Post-restore verification: diff -r between restored dest and source"
                )
                diff2 = run(
                    f"diff -r {shlex_quote(dest)} {shlex_quote(src_for_copy)}",
                    capture=True,
                    check=False,
                )
            ...
            if not getattr(args, "dry_run", False):
                ...
                if email_before:
                    cprint(
                        NEON_YELLOW,
                        f"Auto-adding 24h cooldown for outgoing account: {email_before}",
                    )
...
def main():
    ...
    p.add_argument(
        "--from-dir",
        help="Directory backup to restore from (legacy, archive is preferred)",
    )
    p.add_argument("--from-archive", help="Tar.gz archive to restore from")
    p.add_argument(
        "--search-dir",
        default=DEFAULT_BACKUP_DIR,
        help="Directory to search for timestamped backups when no source is specified (default: ~/.gemini-manager/backups)",
    )
    p.add_argument(
        "--dest",
    ...
    p.add_argument(
        "--auto",
        action="store_true",
        help="Automatically restore the next best available account",
    )
    args = p.parse_args()
