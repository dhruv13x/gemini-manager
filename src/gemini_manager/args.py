...
class RichHelpParser(argparse.ArgumentParser):
    ...
    def print_help(self, file=None):
        ...
        # Usage
        console.print(
            f"[bold white]Usage:[/ ] [dim]{self.format_usage().strip().replace('usage: ', '')}[/]\n"
        )
        ...
        console.print(
            Panel(
                table, title="[bold green]Arguments & Options[/]", border_style="cyan"
            )
        )
...
def get_parser() -> argparse.ArgumentParser:
    ...
    subparsers = parser.add_subparsers(
        dest="command", help="Available commands", parser_class=RichHelpParser
    )
    ...
    parser.add_argument(
        "--session", action="store_true", help="Show current active session"
    )
    parser.add_argument(
        "--update", action="store_true", help="Reinstall / update Gemini CLI"
    )
    parser.add_argument("--check-update", action="store_true", help="Check for updates")
    parser.add_argument(
        "--profile",
        help="Specify a configuration profile to use (e.g., work, personal)",
    )

    # Backup command
    backup_parser = subparsers.add_parser(
        "backup",
        help="Backup Gemini configuration and chats (local or Backblaze B2 cloud).",
    )
    backup_parser.add_argument(
        "--src",
        default="~/.gemini-manager",
        help="Source gemini-manager dir (default ~/.gemini-manager)",
    )
    backup_parser.add_argument(
        "--archive-dir",
        default=DEFAULT_BACKUP_DIR,
        help="Directory to store tar.gz archives (default: ~/.gemini-manager/backups)",
    )
    backup_parser.add_argument(
        "--dest-dir-parent",
        default=OLD_CONFIGS_DIR,
        help="Parent directory where timestamped directory backups are stored",
    )
    backup_parser.add_argument(
    ...
    backup_parser.add_argument(
        "--b2-id", help="B2 Key ID (or set env GEMINI_B2_KEY_ID)"
    )
    backup_parser.add_argument(
        "--b2-key", help="B2 App Key (or set env GEMINI_B2_APP_KEY)"
    )

    # Restore command
    restore_parser = subparsers.add_parser(
        "restore",
        help="Restore Gemini configuration from a backup (local or Backblaze B2 cloud).",
    )
    restore_parser.add_argument(
        "--from-dir", help="Directory backup to restore from (preferred)"
    )
    restore_parser.add_argument("--from-archive", help="Tar.gz archive to restore from")
    restore_parser.add_argument(
        "--search-dir",
        default=DEFAULT_BACKUP_DIR,
        help="Directory to search for backup archives (*.gemini-manager.tar.gz) when no --from-dir (default: ~/.gemini-manager/backups)",
    )
    restore_parser.add_argument(
        "--dest",
        default="~/.gemini-manager",
        help="Destination (default ~/.gemini-manager)",
    )
    restore_parser.add_argument(
        "--force",
        action="store_true",
        help="Allow destructive replace without keeping .bak",
    )
    restore_parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Do a dry run without destructive actions",
    )
    restore_parser.add_argument(
        "--cloud", action="store_true", help="Restore from Cloud (B2)"
    )
    ...
    restore_parser.add_argument(
        "--auto",
        action="store_true",
        help="Automatically restore the best available account",
    )

    # Chat command
    chat_parser = subparsers.add_parser("chat", help="Manage chat history.")
    chat_subparsers = chat_parser.add_subparsers(
        dest="chat_command", help="Chat commands"
    )
    chat_subparsers.add_parser(
        "backup", help="Backup chat history."
    )
    chat_subparsers.add_parser(
        "restore", help="Restore chat history."
    )
    chat_cleanup_parser = chat_subparsers.add_parser(
        "cleanup", help="Clear temporary chat history and logs."
    )
    ...
    chat_subparsers.add_parser(
        "resume", help="Resume the last chat session."
    )

    # Integrity check command
    integrity_parser = subparsers.add_parser(
        "check-integrity",
        help="Check integrity of current configuration against the latest backup.",
    )
    integrity_parser.add_argument(
        "--src",
        default="~/.gemini-manager",
        help="Source directory for integrity check (default: ~/.gemini-manager)",
    )

    # List backups command
    list_backups_parser = subparsers.add_parser(
        "list-backups", help="List available backups (local or Backblaze B2 cloud)."
    )
    list_backups_parser.add_argument(
        "--search-dir",
        default=DEFAULT_BACKUP_DIR,
        help="Directory to search for backup archives (default: ~/.gemini-manager/backups)",
    )
    list_backups_parser.add_argument(
        "--cloud", action="store_true", help="List backups from Cloud (B2)"
    )
    ...
    # Check B2 command
    check_b2_parser = subparsers.add_parser(
        "check-b2", help="Verify Backblaze B2 credentials."
    )
    check_b2_parser.add_argument(
        "--b2-id", help="B2 Key ID (or set env GEMINI_B2_KEY_ID)"
    )
    check_b2_parser.add_argument(
        "--b2-key", help="B2 App Key (or set env GEMINI_B2_APP_KEY)"
    ...
    check_b2_parser.add_argument(
        "--bucket", help="B2 Bucket Name (or set env GEMINI_B2_BUCKET)"
    )
    ...
    sync_subparsers = sync_parser.add_subparsers(
        dest="sync_direction", help="Sync direction"
    )

    # Sync Push (Local -> Cloud)
    push_parser = sync_subparsers.add_parser(
        "push", help="Upload missing local backups to Cloud."
    )
    push_parser.add_argument(
        "--backup-dir",
        default=DEFAULT_BACKUP_DIR,
        help="Local backup directory (default: ~/.gemini-manager/backups)",
    )
    ...
    pull_parser = sync_subparsers.add_parser(
        "pull", help="Download missing Cloud backups to local."
    )
    ...
    config_parser = subparsers.add_parser(
        "config", help="Manage persistent configuration."
    )
    config_parser.add_argument(
        "config_action",
        choices=["set", "get", "list", "unset", "init"],
        nargs="?",
    ...
    config_parser.add_argument(
        "--force",
        action="store_true",
        help="Force save sensitive keys without confirmation (automation mode)",
    )
    config_parser.add_argument(
        "--init", action="store_true", help="Run the interactive configuration wizard"
    )

    # Resets command (New subcommand for reset management)
    resets_parser = subparsers.add_parser(
        "resets", help="Manage Gemini free tier reset schedules."
    )
    resets_parser.add_argument(
        "--list", action="store_true", help="List saved schedules"
    )
    resets_parser.add_argument(
        "--next",
        nargs="?",
        const="*ALL*",
        help="Show next usage time. Optionally pass email or id.",
    )
    resets_parser.add_argument(
        "--add",
        nargs="?",
        const="",
        help="Add time manually. Example: --add '01:00 PM user@example.com'",
    )
    resets_parser.add_argument(
        "--remove", nargs=1, help="Remove saved entry by id or email."
    )
    ...
    prune_parser.add_argument(
        "--cloud", action="store_true", help="Prune both local AND cloud backups"
    )
    prune_parser.add_argument(
        "--cloud-only", action="store_true", help="Only prune cloud backups"
    )
    prune_parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    cooldown_parser = subparsers.add_parser(
        "cooldown", help="Show account cooldown status, with optional cloud sync."
    )
    cooldown_parser.add_argument(
        "--cloud", action="store_true", help="Sync cooldown status from the cloud."
    )
    ...
    cooldown_parser.add_argument(
        "--remove",
        nargs=1,
        help="Remove an account from the dashboard (both cooldown and resets).",
    )
    cooldown_parser.add_argument(
        "--reset-all",
        action="store_true",
        help="⚠️ Clear ALL account activity and reset data (Local & Cloud).",
    )
    ...
    subparsers.add_parser(
        "recommend",
        aliases=["next"],
        help="Suggest the next best account (Green & Least Recently Used).",
    )
    # No arguments needed for now, but could add specific filters later.

    # Stats command
    subparsers.add_parser(
        "stats", aliases=["usage"], help="Show usage statistics (last 7 days)."
    )
