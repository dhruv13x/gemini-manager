...
import os
from gemini_manager.prune import (
    do_prune,
    get_backup_list,
    get_backup_list_dirs,
    prune_list,
    parse_ts,
)
...
def test_parse_ts():
    ...
    assert (
        parse_ts("2023-01-01_120000-backup.gemini-manager") is not None
    )  # Directory format
...
def test_do_prune_local(mock_cprint, fs):
    ...
    fs.create_file(
        os.path.join(archive_dir, "2023-01-01_100000-u.gemini-manager.tar.gz")
    )
    fs.create_file(
        os.path.join(archive_dir, "2023-01-02_100000-u.gemini-manager.tar.gz")
    )
    fs.create_file(
        os.path.join(archive_dir, "2023-01-03_100000-u.gemini-manager.tar.gz")
    )
    ...
    assert not os.path.exists(
        os.path.join(dir_backup_path, "2023-01-01_110000-u.gemini-manager")
    )
    assert not os.path.exists(
        os.path.join(dir_backup_path, "2023-01-02_110000-u.gemini-manager")
    )
    assert os.path.exists(
        os.path.join(dir_backup_path, "2023-01-03_110000-u.gemini-manager")
    )  # Kept
...
def test_do_prune_cloud(mock_cprint, mock_b2_cls, mock_creds, fs):
    ...
    args = mock_args(
        keep=1, cloud=True, backup_dir="/tmp/nonexistent"
    )  # Local part will be skipped
    ...
    mock_b2.bucket.delete_file_version.assert_called_once_with(
        "id1", "2023-01-01_100000-u.gemini-manager.tar.gz"
    )
...
def test_do_prune_cloud_no_creds(mock_cprint, mock_creds, fs):
    ...
    do_prune(args)
    assert any(
        "Skipping (credentials not set)." in str(args)
...
def test_do_prune_cloud_only_no_creds_error(mock_cprint, mock_creds, fs):
    ...
    # Error printed
    assert any(
        "Cloud credentials missing." in str(args) for args in mock_cprint.call_args_list
    )
...
def test_do_prune_local_remove_fail(mock_cprint, fs):
    ...
    for call_args in mock_cprint.call_args_list:
        ...
        if (
            "Failed to remove" in arg_str
            and "2023-01-01_100000-u.gemini-manager.tar.gz" in arg_str
        ):
            file_err_found = True
        if (
            "Failed to remove directory" in arg_str
            and "2023-01-01_110000-u.gemini-manager" in arg_str
        ):
            dir_err_found = True

    assert file_err_found, (
        f"File removal error not found in cprint calls: {mock_cprint.call_args_list}"
    )
    assert dir_err_found, (
        f"Directory removal error not found in cprint calls: {mock_cprint.call_args_list}"
    )
...
def test_do_prune_cloud_delete_fail(mock_cprint, mock_b2_cls, mock_creds, fs):
    ...
    assert any(
        "Failed to delete cloud file 2023-01-01_100000-u.gemini-manager.tar.gz"
        in str(args)
        for args in mock_cprint.call_args_list
    )
