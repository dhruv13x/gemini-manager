...
from gemini_manager.reset_helpers import (
    ...
    add_reset_entry,
    save_reset_time_from_output,
    _compute_next_local_for_time,
    cleanup_expired,
    do_list_resets,
    do_next_reset,
    do_capture_reset,
...
def test_run_cmd_safe_exception_capture(mocker):
    ...
    mock_save = mocker.patch("gemini_manager.reset_helpers.save_reset_time_from_output")
    with patch(
        "subprocess.run", side_effect=Exception("Access resets at 10:00 AM UTC")
    ):
        run_cmd_safe("cmd")
        mock_save.assert_called()
...
def test_add_reset_entry_valid(fs):
    with patch(
        "gemini_manager.reset_helpers._now_local",
        return_value=datetime(2023, 1, 1, 9, 0, 0).astimezone(),
    ):
...
def test_cleanup_expired(fs):
    ...
    data = [{"reset_ist": old, "id": "1"}, {"reset_ist": future, "id": "2"}]
    fs.create_file(RESETS_FILE, contents=json.dumps(data))
...
def test_save_reset_time_from_output(fs, capsys):
    with patch(
        ...
        res = save_reset_time_from_output(
            "Access resets at 10:00 AM UTC test@example.com"
        )
        assert res is True
...
def test_do_capture_reset_arg(fs, capsys):
    # Using argument
    with patch(
        "gemini_manager.reset_helpers.save_reset_time_from_output", return_value=True
    ):
...
def test_do_capture_reset_stdin(fs, capsys):
    ...
    with patch("gemini_manager.reset_helpers.sys.stdin") as mock_stdin:
        ...
        with patch(
            "gemini_manager.reset_helpers.save_reset_time_from_output",
            return_value=True,
        ):
            do_capture_reset()
...
def test_save_reset_time_from_output_exception(fs, capsys):
    with patch(
        "gemini_manager.reset_helpers.add_reset_entry",
...
def test_do_next_reset_found(fs, capsys):
    ...
    data = [{"reset_ist": future, "id": "1", "email": "test"}]
    with patch(
        "gemini_manager.reset_helpers._load_and_cleanup_store", return_value=data
    ):
        do_next_reset()
    captured = capsys.readouterr()
    assert "Next reset for test" in captured.out
...
def test_do_capture_reset_invalid(fs, capsys):
    with patch(
        "gemini_manager.reset_helpers.save_reset_time_from_output", return_value=False
    ):
...
def test_load_and_cleanup_store(fs):
    # Test wrapper
    with patch(
        "gemini_manager.reset_helpers.cleanup_expired", return_value=[]
    ) as mock_clean:
        with patch(
            "gemini_manager.reset_helpers._load_store", return_value=[]
        ) as mock_load:
