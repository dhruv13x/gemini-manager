...
def test_backup_encrypt_calls_gpg(mocker, mock_subprocess):
    ...
    mocker.patch(
        "gemini_manager.backup.read_active_email", return_value="test@example.com"
    )
    ...
    mocker.patch(
        "gemini_manager.backup.os.remove"
    )  # Mock remove to prevent FileNotFoundError
    ...
    with unittest.mock.patch.dict(
        os.environ, {"GEMINI_BACKUP_PASSWORD": "password123"}
    ):
        perform_backup(args)
...
def test_restore_decrypt_calls_gpg(mocker, mock_restore_subprocess):
    ...
    mocker.patch(
        "gemini_manager.restore.tempfile.mkdtemp", return_value="/tmp/work_tmp"
    )
    ...
    mocker.patch(
        "gemini_manager.restore.os.listdir",
        return_value=["2025-01-01_120000-test@example.com.gemini-manager.tar.gz.gpg"],
    )
