import pytest
from gemini_manager.wizard import run_wizard
...
def mock_set_setting(mocker):
    ...
    return mocker.patch("gemini_manager.wizard.set_setting")
...
def mock_cprint(mocker):
    ...
    return mocker.patch("gemini_manager.wizard.cprint")
...
def test_run_wizard_success(mock_prompt_ask, mock_set_setting, mock_cprint):
    ...
    mock_prompt_ask.assert_any_call(
        f"[{NEON_YELLOW}]Enter B2 Bucket Name[/{NEON_YELLOW}]"
    )
    mock_prompt_ask.assert_any_call(
    ...
    mock_set_setting.assert_any_call(
        "GEMINI_BACKUP_DIR", "/custom/backup/dir"
    )  # Added assertion

    # Verify console output
    mock_cprint.assert_any_call(
        NEON_CYAN, "Welcome to the Gemini CLI Configuration Wizard!"
    )
...
def test_run_wizard_with_default_backup_dir(
    mock_prompt_ask, mock_set_setting, mock_cprint
):
    """
    ...
    mock_set_setting.assert_any_call(
        "GEMINI_BACKUP_DIR", DEFAULT_BACKUP_DIR
    )  # Added assertion
...
def test_run_wizard_no_b2_credentials(mock_prompt_ask, mock_set_setting, mock_cprint):
    ...
    mock_set_setting.assert_any_call(
        "GEMINI_BACKUP_DIR", "/custom/backup/dir_empty"
    )  # Assert only this call
